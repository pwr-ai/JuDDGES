    ...
    try:
        ...
        with StreamingIngester() as ingester:
            ...
            if comprehensive_stats.get("document_types"):
                most_common_type = max(comprehensive_stats["document_types"].items(), key=lambda x: x[1])
                console.print(f"Most common document type: {most_common_type[0]} ({most_common_type[1]:,} documents)")
            ...
            if date_stats.get("total_with_dates", 0) > 0:
                console.print(f"Document date range: {date_stats['earliest_date']} to {date_stats['latest_date']}")
