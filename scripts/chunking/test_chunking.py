...
def main():
    ...
    results = {
        ...
        "original_text": {
            ...
            "text_preview": judgment_text[:500] + "..." if len(judgment_text) > 500 else judgment_text
        },
        "chunks": []
    }
    ...
    for i, (chunk_id, chunk_text, chunk_len) in enumerate(zip(
        chunk_data["chunk_id"], 
    ...
    console.print(f"- Average chunk size: {sum(c['char_length'] for c in results['chunks']) / len(results['chunks']):.1f} chars")
    console.print(f"- Average tokens per chunk: {sum(c['token_count'] for c in results['chunks']) / len(results['chunks']):.1f}")
    console.print(f"- Token range: {min(c['token_count'] for c in results['chunks'])}-{max(c['token_count'] for c in results['chunks'])} tokens")
