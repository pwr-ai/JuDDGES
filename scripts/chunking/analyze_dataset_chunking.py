...
""",
    ...
    parser.add_argument(
        "--num-examples", type=int, default=100, help="Number of examples to process (default: 100)"
    )
...
def analyze_chunking_statistics(
    dataset_name,
    num_examples=100,
    output_prefix="dataset_chunking_analysis",
    chunk_size=512,
    overlap=128,
    ...
    for doc in track(dataset, total=num_examples, description="Processing documents"):
        ...
        try:
            # Apply chunking
            chunk_data = chunker({"document_id": [doc_id], "text": [judgment_text]})
...
def display_results(stats):
    ...
    console.print(
        f"Average chunks per document: {stats['dataset_info']['total_chunks'] / stats['dataset_info']['processed_documents']:.1f}"
    )
    ...
    console.print(
        f"• Average chunk uses {avg_tokens:.0f} tokens ({avg_tokens/max_tokens*100:.1f}% of max capacity)"
    )

    if chunk_stats["token_count"]["max"] > max_tokens:
        console.print(
            f"• [red]Warning: Some chunks exceed token limit! Max: {chunk_stats['token_count']['max']} tokens[/red]"
        )
    else:
        console.print(
            f"• [green]✓ All chunks within token limit (max: {chunk_stats['token_count']['max']} tokens)[/green]"
        )

    efficiency = (chunk_stats["token_count"]["mean"] / max_tokens) * 100
    if efficiency < 70:
        console.print(
            f"• [yellow]Low token efficiency ({efficiency:.1f}%) - consider reducing chunk size[/yellow]"
        )
    elif efficiency > 95:
        console.print(
            f"• [yellow]High token efficiency ({efficiency:.1f}%) - chunks are near capacity[/yellow]"
        )
    else:
