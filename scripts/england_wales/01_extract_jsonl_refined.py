...
def extract_appeal_type(text):
    patterns = [
        (
            r"appeal\s+against\s+\S+\s+sentence\s+or\s+\S+\s+conviction",
            "conviction_sentence",
        ),
        (
            r"appeal\s+against\s+\S+\s+conviction\s+or\s+\S+\s+sentence",
            "conviction_sentence",
        ),
...
def extract_and_clean_judges(paragraphs):
    ...
    for para in paragraphs:
        ...
        if re.search(
            ...
            if (
                cleaned_text
                and "Royal Courts of Justice" not in cleaned_text
                and cleaned_text != "THE LORD CHIEF JUSTICE OF ENGLAND AND WALES"
            ):
...
def extract_information_from_xml(xml_content, file_name):
    ...
    publication_date = (
        soup.find("FRBRdate", {"name": "judgment"})["date"]
        if soup.find("FRBRdate", {"name": "judgment"})
        else None
    )
    ...
    judges = [
        ...
        and re.search(
            r"\bJustice\b|\bJudge\b|\bSIR\b|\bHonour\b|\bHHJ\b",
            judge["showAs"],
            re.IGNORECASE,
        )
    ]
    ...
    uri = (
        soup.find("FRBRWork").find("FRBRuri")["value"]
        if soup.find("FRBRWork") and soup.find("FRBRWork").find("FRBRuri")
        else None
    )
...
def process_directory(directory_path, output_file):
    ...
    with Pool() as pool, open(output_file, "w") as jsonl_file:
        for judgment_data in tqdm(
            pool.imap(process_file, xml_files),
            total=len(xml_files),
