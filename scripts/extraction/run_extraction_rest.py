...
def fetch_documents_rest(
    ...
    """Fetch documents using Weaviate REST API directly.
    ...
    # Fetch documents with GraphQL query (REST endpoint)
    fetch_size = sample_size * 5  # Fetch more to filter for valid full_text

    logger.info(f"Fetching up to {fetch_size} documents from {base_url} (REST API)...")
    ...
    try:
        ...
        logger.info(f"Fetched {len(documents)} documents from Weaviate")
